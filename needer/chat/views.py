    ...
    def get_context_data(self, **kwargs):
        ...
        if thread.first_person == user:
            if thread.closed_by_first_user:
                context['messages'] = ChatMessage.objects.filter(Q(timestamp__gte=thread.closed_by_first_user))
            else:
                context['messages'] == ChatMessage.objects.filter(thread=thread).order_by('-timestamp')
        elif thread.second_person == user:
            if thread.closed_by_second_user:
                context['messages'] = ChatMessage.objects.filter(Q(timestamp__gte=thread.closed_by_second_user))
            else:
                context['messages'] == ChatMessage.objects.filter(thread=thread).order_by('-timestamp')
